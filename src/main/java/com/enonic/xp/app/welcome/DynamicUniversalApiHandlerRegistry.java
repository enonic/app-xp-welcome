...
{
    private final CopyOnWriteArrayList<DynamicUniversalApiHolder> dynamicApiHandlers = new CopyOnWriteArrayList<>();
    ...
    {
        return dynamicApiHandlers.stream().map( DynamicUniversalApiHolder::getApiDescriptor ).collect( Collectors.toList() );
    }
    ...
    public void addApiHandler( final UniversalApiHandler apiHandler, final Map<String, ?> properties )
    {
        ...
        this.dynamicApiHandlers.add( new DynamicUniversalApiHolder( apiHandler, apiDescriptor ) );
    }
    ...
    private ApiDescriptor createDynamicApiDescriptor( final Map<String, ?> properties )
    {
        ...
        {
            builder.mount( resolveDynamicMount( properties.get( "mount" ) ) );
        }

        return builder.build();
    }

    private String[] resolveDynamicMount( final Object mount )
    {
        return switch ( mount )
        {
